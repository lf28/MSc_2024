...
**Abstract** *We review some basic maths techniques/concepts for frequentist's machine learning. We focus on maximum likelihood estimation (MLE) and its related techniques: specifically, conditional expectation, gradient descent, Newton's method, reparameterisation trick.*
...
$$y = \mathbf{x}^\top \boldsymbol{\beta} + \xi, \;\;\text{where}\;\; \xi \sim \mathcal{N}(0, \sigma^2),$$

Given data $\mathbf{y} = [y_1, y_2, \ldots, y_n]^\top$ and $\mathbf{X} = [\mathbf{x}_1, \mathbf{x}_2, \ldots, \mathbf{x}_n]^\top$ (the data is given below);

*  what is $\mathbb{E}[y|\mathbf{x}]$? 
...
*  maximise the log-likelihood function by (you should implement the algorithm in Julia)
  ...
  * gradient descent
  * Newton's method (check CS5014 lecture notes if you do not know Newton's method)
...
$$P(y|\mathbf{x}) = \text{Bernoulli}(\sigma(\mathbf{x}^\top \boldsymbol{\beta})),$$
Given data $\mathbf{y} = [y_1, y_2, \ldots, y_n]^\top$ where $y_i \in \{0, 1\}$ and $\mathbf{X} = [\mathbf{x}_1, \mathbf{x}_2, \ldots, \boldsymbol{x}_n]^\top$;

*  what is $\mathbb{E}[y|\mathbf{x}]$ ? Plot $E[y|\mathbf{x}]$ by 3-d surface or 2-d contour plot.
*  what are the paremeter for this model ?
*  what is the likelihood function ?
*  simulate $\mathbf{y}$ based on the given model ($\boldsymbol{\beta}$ and $\mathbf{X}$ are already given below)
*  maximise the log-likelihood function by (you should implement the algorithm in Julia)
  * gradient descent
  * Newton's method
"""
