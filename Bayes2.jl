...
$$q(\theta'|\theta) = \mathcal{N}\left (\theta, \left(C_0^{-1} + \frac{6}{\pi^2} \mathbf X^\top \mathbf X\right)^{-1}\right),$$
   where $C_0$ is the prior's variance: 

$$p(\theta)= \mathcal{N}(m_0, C_0),$$ and $m_0=\mathbf 0$ to achieve some shrinkage effect.


What do you observe if we use the prior as the Gaussian proposal ?, i.e.

$$q(\theta'|\theta) = \mathcal{N}(m_0, C_0)$$
...
$p(\boldsymbol \theta|\boldsymbol \lambda^2)=\mathcal{N}(\mathbf 0, \text{diag}(\boldsymbol \lambda^2))=\prod_{d=1}^D \mathcal{N}(\theta_d; 0, \lambda_d^2) ;$ 
...
$$p(\mathbf y|\Phi, \boldsymbol \theta, \sigma^2) = \mathcal{N}(\mathbf y; \Phi\boldsymbol \theta, \sigma^2I_N)$$
...
manifest_format = "2.0"
project_hash = "567ad5dff479f68f3b0181de0886f2754e58f548"
