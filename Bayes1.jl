...
1. Consider the following regression dataset with $N=50$ observations, and `xsamples` as input and `ts` as targets. Apply Radial basis function (RBF) to expand the design matrix from $\mathbb{R}^1$ to $\mathbb{R}^{50}:$ $$\text{rbf}(x; \mu, \sigma) = \text{exp}\left (- \frac{(x-\mu)^2}{2\sigma^2}\right ),$$
