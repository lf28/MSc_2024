...
*Fixed basis expansion* (read Chapter 7.2 of MLAPP or Bishop's chapter 3) can be viewed as a Neural Net's hidden layer. The difference is: the expansion basis function do not change and assumed known for the fixed basis expansion model. And there is no fundamental difference between these two.

1. Consider the following regression dataset with $N=50$ observations, with `xsamples` (input) and `ts` (targets). Apply Radial basis function (RBF) to expand the design matrix from $R^1$ to $R^{50}:$ $$\text{rbf}(x; \mu, \sigma) = \text{exp}\left (- \frac{(x-\mu)^2}{2\sigma^2}\right ),$$
   * choose the expansion locations as `xsamples`
   * and fix $\sigma^2 = 0.5$

2. Fit the regression by using MLE and 
