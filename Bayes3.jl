...
Mean field VI assumes $q(w) = \prod_{i} q(w_i)$, i.e. the approximating variational distribution is fully independent. Then variational calculus tells us the best approximating distrbution can be found by setting each dimension of $q$, $q(w_i)$ by the expectation of the log joint distribution: i.e.

for $i \in 1, \ldots, d$

$$q(w_i) = \mathbb{E}_{\{w_{j, j\neq i}\}}[\ln p(\mathcal D, w)].$$ 
...
$$p(\mathbf y| \Phi,  \theta,\beta) = \mathcal{N}(\Phi  \theta, \beta^{-1}\mathbf I_{N})$$
and we further impose the following (hyper-)priors on the unknown parameters: 

$p(\theta) = \mathcal{N}(\boldsymbol 0, \text{diag}(\boldsymbol \alpha)^{-1}) = \prod_{d=1}^D \mathcal{N}(\theta_d; 0, \alpha_d^{-1})$
...
begin
	...
	scatter!(xsamples, ts, markershape=:xcross, markersize=2.5, markerstrokewidth=0.8, markercolor=:gray, markeralpha=0.9, label="Obvs")
...
manifest_format = "2.0"
project_hash = "3a5e401b0dcec5fbd6b62f08e36d1ebf48298597"
